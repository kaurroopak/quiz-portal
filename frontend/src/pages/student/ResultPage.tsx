...
export default function ResultPage() {
  ...
  const pct = session.totalMarks ? Math.round((session.score / session.totalMarks) * 100) : 0;
      ...
      <div className="page">
        <div className="container">
          <div className="result-card">
            ...
            <div className="result-score">{session.score}<span style={{ fontSize: 32, opacity: 0.5 }}>/{session.totalMarks}</span></div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, background: 'var(--bg-surface)', borderRadius: 10, padding: 16, marginBottom: 24, textAlign: 'left' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                ...
                <span>{session.submittedAt ? new Date(session.submittedAt).toLocaleString() : '—'}</span>
              </div>
            </div>
